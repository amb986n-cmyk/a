...
ssh_client(){
 ...
 ssh -t $U@$IP "bash -lc 'sudo apt update && sudo apt install -y nagios-nrpe-server nagios-plugins wget ufw && sudo sed -i \"s/^allowed_hosts=.*/allowed_hosts=127.0.0.1,::1,$SIP/\" /etc/nagios/nrpe.cfg && sudo sed -i \"s/^server_address=.*/#server_address=/\" /etc/nagios/nrpe.cfg && echo command[check_load]=/usr/lib/nagios/plugins/check_load -w 3,2.5,2 -c 6,5,4 | sudo tee -a /etc/nagios/nrpe.cfg >/dev/null && echo command[check_disk]=/usr/lib/nagios/plugins/check_disk -w 15% -c 5% -p / | sudo tee -a /etc/nagios/nrpe.cfg >/dev/null && wget -qO /tmp/check_mem.pl https://raw.githubusercontent.com/justintime/nagios-plugins/master/check_mem/check_mem.pl && sudo mv /tmp/check_mem.pl /usr/lib/nagios/plugins/check_mem && sudo chmod +x /usr/lib/nagios/plugins/check_mem && echo command[check_mem]=/usr/lib/nagios/plugins/check_mem -u -w 80 -c 90 | sudo tee -a /etc/nagios/nrpe.cfg >/dev/null && sudo ufw allow 5666/tcp || true && sudo systemctl enable --now nagios-nrpe-server && sudo systemctl restart nagios-nrpe-server && sudo ss -tulpn | grep 5666'"
